...
func _ready():
	...
	font_holder.font_data = load("res://Art/Fonts/m3x6.ttf")
...
func _draw():
	...
	if turny > 0:
		for i in range(len(test_page)):
			draw_string(font_holder, Vector2(1, 12*i+21), test_page[i], Color(0.1, 0.1, 0.1))
	else:
		...
		for i in range(len(test_page_2)):
			draw_string(font_holder, Vector2(1, 12*i+21), test_page_2[i], Color(0.1, 0.1, 0.1))
