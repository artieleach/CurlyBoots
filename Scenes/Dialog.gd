...
##### SETUP #####
## Paths ##
var dialogs_folder = 'res://Writing/Dialog' # Folder where the JSON files will be stored
var choice_scene = load('res://Scenes/Choice.tscn') # Base scene for que choices
## Required nodes ##
...
onready var portrait : Node = $Portrait # Used for showing the avatar on the dialog
...
var characters_folder = 'res://Art/Portraits'
...
func set_frame(): # Mostly aligment operations.
	...
	portrait.modulate = white_transparent
...
func animate_sprite(_direction, image, animation):
	var current_pos
	var move_vector
...
func load_image(changed_sprite, image):
	changed_sprite.texture = load('%s/%s.png' % [characters_folder, image])
